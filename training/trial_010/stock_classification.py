...
from net import MyChainLSTM, MyChainBiDirectionalLSTM, MyChainLSTM3
...
def save_model(model, path, device):
    ...
    if device >= 0:
        model2.to_cpu()
        serializers.save_npz(path, model2)
        model2.to_gpu()
    else:
        serializers.save_npz(path, model2)
...
def out_accuracy(model, data_iter, n_test_data, dst_path, device):
    ...
    def make_accuracy(trainer):
        ...
        stock_validation.summarise_result2(model, data_iter, n_test_data, os.path.join(dst_path, iter_str), device)
...
def main():
    ...
    #model = MyChainLSTM(in_size=9, hidden_size=10, seq_size=20, out_size=len(y_axis),
    #                    device=args.gpu, lstm_do_ratio=0.1, affine_do_ratio=0.0)
    model = MyChainLSTM3(in_size=9, hidden_size=5, seq_size=20, out_size=3,
                        device=args.gpu, lstm_do_ratio=0.1, affine_do_ratio=0.5)
    ...
    train_data = stock_dataset.MyStockDataset3(train_pairs, y_axis)
    test_data = stock_dataset.MyStockDataset3(test_pairs, y_axis)
